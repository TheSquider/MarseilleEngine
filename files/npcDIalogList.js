const toRead = 
"NW 0 # ERROR NO VALID DIALOG, VAR InternalID RETURNED 0 AT npc.js. The ID give to the NPC does not coralate to an existing ID in the npcDIalogList.js file. Check the IDs you've declared." + 
"NW 1 # I hate taxes # That is why I dont pay them # I will die # Oh well! # CMD#console.log('end it')CMD#Yep... # . # .. # ..." +
"NW 2 # I had forgotten about myself. # I honestly thought I was NPC45, I talked like them, I walked like them # I tried to think of my body but only theirs came up. # I know it's not my fault, I know that the code was ok, I know it was HIS fault, HE made me forget. Why the fuck can't he remember a single fucking thing stupid bitch. It's HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT HIS FAULT # I don't know if this is my house. # ... # Before you leave, I have a quest. # Give this to NPC45" +
"NW 4 # It's a tree. # You don't know why you keep staring at it..." +
"NW 6 # It's a tree. # You don't know why you keep staring at it... # ...Actually it wants to tell you something! # (Judging by the fact that none of us actually are self aware, isn't what NPC 2 has to say a bit cringe?) # (The dev just used an NPC to attack himself about an issue he has gotten over. Why?) # (It's not even that important)" +
"NW 5 # Depression is an upward battle you win by being hunan. # For me being human means creating art and connecting with others through it. # Depression is an upward battle I win by creating towards the sky." +
"NW 45 # I hate the fact that Im forced to look like §. # Actually I dont have to be § anymore # The dude updated the code! # Actually never mind it's broken now. # This place is really a testament to development huh?" +
"NW 89 # I # AM # SO # HAPPY # CMD#document.body.style.background = 'red'CMD#MUAHAHAHHA # ... # CMD#document.body.style.background = '#ffdead'CMD#Ok I'm sorry..." +


"NW 99 # Controls are: WASD to move, Q to interact, P to open the menu, [ and ] to navigate, and O to select." +
"NW 100 # Everything you see in changable. You can work within the guidelines I've given you or colour outside the page. Full documentatiion comes with the free download. # See this more of a jumping board for ideas, not a limitation. I've tried to make this engine simple to work with while still letting people mess with the code itself." +
"NW 101 # Art and software should be free, and above all human. If you end up making anything with this engine please don't monitise it and above all share it. # I will be hearing feedback and the GitHub link is on the itch.io page. # Email me (and I will respond) at frechsquid.headcanon@gmail.com" +
"NW 102 # If you press Q again you will go to a test game to see more of the capabilities of the engine, press P to quit. # CMD#window.location.href = './index.html'CMD#" +

"NW 666 # I used to do something debug related but I was replaced by the console. # Even though I just occupy code now, I still enjoy looking at the other NPCs doing what they are meant to be doing. # One day I'll get reporpused. # One day I'll get reporpused.. # One day I'll get reporpused..."